...
def softmax_loss(x, y):
    ...
    dx = torch.exp(logits) - torch.nn.functional.one_hot(y, num_classes=x.shape[1]).reshape(x.shape)
    dx = dx / x.shape[0]
    ######################################################################
    #                END OF YOUR CODE                                    #
    ######################################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # scores variable.                                          #
        #############################################################
        h1, cache_1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        scores, cache_2 = Linear.forward(h1, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss += self.reg * torch.sum(torch.square(self.params['W1']))
        loss += self.reg * torch.sum(torch.square(self.params['W2']))
        dout, grads['W2'], grads['b2'] = Linear.backward(dout, cache_2)
        dout, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            in_dim, out_dim = all_dims[i:i + 2]
            self.params[f'W{i + 1}'] = weight_scale * torch.randn((in_dim, out_dim), dtype=dtype, device=device)
            self.params[f'b{i + 1}'] = torch.zeros(size=(out_dim,), dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                out, cache = Linear.forward(out, w, b)
            else:
                out, cache = Linear_ReLU.forward(out, w, b)
                if self.use_dropout:
                  out, cache_2 = Dropout.forward(out, self.dropout_param)
                  cache = (cache, cache_2)
            caches.append(cache)
        scores = out
        #################################################################
        ...
        loss, dout = softmax_loss(scores, y)
        for i in range(self.num_layers):
            loss += 0.5 * self.reg * torch.sum(torch.square(self.params[f'W{i + 1}']))
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                dout, dw, db = Linear.backward(dout, caches[i])
            else:
                cache = caches[i]
                if self.use_dropout:
                    cache, cache_2 = caches[i]
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            mask = torch.rand(size=x.shape, dtype=x.dtype, device=x.device)
            mask = torch.where(mask > p, torch.ones_like(mask), torch.zeros_like(mask))
            out = x * mask
            out *= 1. / (1. - p)
            ##############################################################
            #                   END OF YOUR CODE                         #
